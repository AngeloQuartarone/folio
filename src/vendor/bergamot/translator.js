/*
 * Vendored from @browsermt/bergamot-translator 0.4.9 (translator.js),
 ...
 *   extensions);
 * - the worker script location comes from `options.workerUrl`, because
 *   `import.meta.url` does not survive bundling to CommonJS.
 */
/**
 ...
 */
const WorkerImpl = (typeof window !== 'undefined' && window.Worker) ? window.Worker : class {
        #worker;
 ...
 */
 export class TranslatorBacking {
     ...
     */
    async loadWorker() {
        const worker = new WorkerImpl(this.options.workerUrl);
